...
> **2026-07-17 追記**
>
> 本文・図・品質説明・講師向け情報は、解説用 PowerPoint と対応しているため維持する。
> 実装確認によって明確化した差分は、既存記述を削除せず、末尾の
> [12. 実装確認後の明確化事項](#12-実装確認後の明確化事項2026-07-17追記) に追記する。
> 既存記述と第12章が競合する箇所は、第12章を現行実装仕様として優先する。
...
12. [実装確認後の明確化事項](#12-実装確認後の明確化事項2026-07-17追記)
   - 12.1 [`order_no` の保持規則](#121-order_no-の保持規則)
   - 12.2 [KPI2 の現行集計定義](#122-kpi2-の現行集計定義)
   - 12.3 [サンプルデータの正本](#123-サンプルデータの正本)
   - 12.4 [自動テストと PostgreSQL 確認](#124-自動テストと-postgresql-確認)
   - 12.5 [ファイル名規則と依存導入](#125-ファイル名規則と依存導入)
...
出荷検査ログの翌営業日補正など、入力元ごとの仕様として補正を認める場合は、補正後の値が期待結果と一致することを確認する。
...
本教材では、SQLAlchemyやPostgreSQLを含む実際の処理経路を確認するため、DB処理そのものをモックやインメモリDBへ置き換えない。

AIにテスト実装を依頼する際は、以下を明記する。

- テスト対象DBには、ローカルのPostgreSQL `results_record_db` を使用する
- DB登録処理をモックへ置き換えない
- テスト開始前のデータ状態を明確にする
- 必要に応じて、テスト前後に対象テーブルを初期化する
...
## 12. 実装確認後の明確化事項（2026-07-17追記）

本章は、既存の解説内容を残したまま、実装・テスト・サンプルデータを再確認して明確になった現行仕様を追記するものである。
本文中の講義用説明、品質論、図、開発初期指示は削除せず、そのまま PowerPoint と対で利用する。

### 12.1 `order_no` の保持規則

正常サンプルCSV、補助マスタ、期待結果CSVを照合した結果、現行サンプルの業務キーは次の形式で統一する。
...
取込時の正規化は**前後空白の除去だけ**とする。

- ハイフンを保持する
- 英字の大文字・小文字を保持する
- 記号を除去しない
- `ORD260105001` と `ORD-260105-001` は別の業務キーとして扱う

これは、記号除去や大文字化によって、本来異なる受注番号を同一視することを防ぐためである。
`order_product_master.csv` も、正常3工程で利用するハイフン付き `order_no` だけを保持する。

### 12.2 KPI2 の現行集計定義

既存の説明では「前工程完了から次工程完了まで」を工程間滞留として扱う箇所がある。
現行実装では、工程で作業中の製番と工程間で未着手の製番を分けるため、次の定義を採用する。

> 工程間滞留とは、前工程が完了し、次工程がまだ開始されていない製番である。
...
- 15分間隔で集計する
- 前日以前から持ち越した未着手製番も含める
- `to_start < from_end` は時系列異常として明細に残し、推移件数から除外する
- 次工程で作業中の製番は工程間滞留に含めない
- `UNIQUE (order_no, process_name)` 前提のため、同一工程の複数候補選択は行わない

KPI2明細の時刻列は `from_end` と `to_start` とする。

### 12.3 サンプルデータの正本

`sample_data/generate_sample_data.py` をサンプルデータ生成の正本とする。
生成スクリプトと配布CSVの世代がずれないよう、CSVを手作業で継ぎ足さない。
...
| 総製番数 | 4,321件 |
| 正常実績 | 3工程すべてに同じ `order_no` が1回ずつ登場 |
| 工程順 | 内装終了 ≤ 外装開始、外装終了 ≤ 出荷検査開始 |
| NG率 | 約2% |

外装組立と出荷検査の作業者能力差は、既存の `make_sample_data.md` に記載した比率を維持する。
生成後は、正常3工程・`order_product_master.csv`・`sample_expected_work_log.csv` の `order_no` が一致することを確認する。
...
### 12.4 自動テストと PostgreSQL 確認

現行の `tests/conftest.py` は、通常の自動テストに SQLite インメモリDBを使用する。
これは取込規則、重複制御、境界条件、KPI計算を高速かつ繰り返し可能に確認するためである。

一方、SQLiteとPostgreSQLには型、DDL、ドライバの差があるため、セミナー前には実際のPostgreSQLで次を別途確認する。

- DDLがエラーなく適用できること
- 正常6ファイルを取り込めること
- 正常件数・reject件数が期待どおりであること
- Streamlitが起動し、KPI 3種を表示できること

したがって、テストは次の二層で扱う。

| 層 | DB | 主な目的 |
|---|---|---|
| 自動テスト | SQLiteインメモリ | 変換・検証・重複・KPIロジックの回帰確認 |
| 結合確認 | PostgreSQL | DDL・型・接続・実画面を含む環境差の確認 |

### 12.5 ファイル名規則と依存導入

内装組立・外装組立の作業者名は、次のファイル名規則から取得する。

```text
INTASM_<作業者>_<YYYYMMまたはYYYYMMDD>.<拡張子>
EXTASM_<作業者>_<YYYYMMまたはYYYYMMDD>.<拡張子>
EXTASM_<ライン>_<作業者>_<YYYYMMまたはYYYYMMDD>.<拡張子>
```

- 拡張子は `.csv`、`.xlsx`、`.xlsm`
- 作業者名は英字に限定せず、日本語も許容する
- `_` は区切り文字として扱う
- 出荷検査は `SHIPCHK_...` とし、作業者は `inspector_name` 列から取得する

依存ライブラリは個別に列挙して導入せず、`requirements.txt` を使用する。

```bash
python -m pip install -r requirements.txt
```

これにより、`altair` 等の導入漏れと、文書間の依存関係差を防ぐ。
