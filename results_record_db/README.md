...
ソフトウェアの品質の話だけでセミナーが成り立つほどの領域ですので、深追いはしません。
...
表を元に完結に整理すると、今回の利用用途を元に考えた場合、今回のプログラムが目指す**レベルは3に相当**します。
ただし、実運用を意図したものではなく、セミナー向けに構築するという前提のため**レベル2の品質**を満たすことを要件とします。

AIコーディングを用いてソフトウェアの品質を高める場合、用途の明示化やテスト要件の明示化などをおこなったうえで、アウトプットされたコードに対して、品質の改善を目的としたアクションが必要です。
AIを用いた開発でも品質要求はそのままコスト(=実装工数)となりますので、便利ツールにミッションクリティカル品質を要求するなどは、適切ではありません。

Codexでコード生成する際に複数案を提示させて妥当な実装を選択する進め方もありますが、作成したコード差分をChatGPTや、Claude code、Code Rabbitなどの
