...
ソフトウェアの品質の話単体で、セミナーが成り立つほどの領域ですので、今回のセミナーでは深追いはしません。私の考えるソフトウェア品質は下記のようにレベル分けされるものです。
...
AIを用いた開発においても、品質要求はそのままコスト(=実装工数)となりますので、便利ツールにミッションクリティカル品質を要求するなどの要求は、適切なアプローチではありません。

また、ソフトウェア品質改善アプローチの手法としてCodexでコード生成する際に複数案を提示させて妥当な実装を選択する進め方もありますが、ソフトウェア開発の知見が十分にない中で比較できない場合、作成したコード差分を**ChatGPTや、Claude code、Code Rabbit**などの他のAIサービスにレビューを依頼し、**多視点からの指摘事項を元に実装品質を高めるアプローチ**も有効です。

いずれも、ソフトウェアを開発する段階でどのような用途で利用するかを、**非機能要件として明示すること**が重要です。
