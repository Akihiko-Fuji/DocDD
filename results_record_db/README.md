...
> **規範性:** 本ドキュメントは仕様書だけでなくセミナーでの解説を兼ねているため、コードの実装・テストの判断は「仕様」のみを正としておこなう。「解説」と「教材コメント」は非規範であり、仕様と矛盾する場合は仕様を優先する。
>
> **仕様の記述規則:** 仕様欄では「できるだけ」「原則として」「必要に応じて」「なるべく」「適切に」「など」「基本的に」を単独で使用しないこととする。これらの語を使用する場合は、適用条件・対象・例外・判定基準を列挙し、実装・テストで YES / NO を判定できる文にする。
>
...
| `END_BEFORE_START` | 内装組立・外装組立で `end_ts < start_ts`（出荷検査は翌営業日補正のため対象外） |
...
`end_ts < start_ts` は出荷検査のreject条件ではない。該当する行は前述の翌営業日補正を適用し、補正後の `end_ts` を用いて取り込む。
...
Streamlit画面は、利用者が実際に見て理解しやすいかを確認しながら改善する領域である。本教材では、初期表示項目と最低限の確認条件を定め、実装後にトライアンドエラーで見せ方を改善する過程も学習対象に含める。

**仕様**

- 画面改善によってKPIの意味を変更してはならない。
