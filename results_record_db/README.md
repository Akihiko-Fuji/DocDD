...
一般的なソフトウェア品質論単体で、セミナーが成り立つほどの領域ですので、今回のセミナーでは深追いはしません。
...
一方で、例外処理 に関しては、ソフトウェア的な知見や慎重さが要求される領域であり、機能要件を整理するだけでは明確になるものではありません。
私は利用用途別に下記のようなレベル分類をしています。以下は一般規格に基づく正式な分類ではなく、本教材で品質要求を説明するための便宜的な分類である。
...
例えば、業務・プロダクト品質であれば、エラーを詳細に分類し必要に応じてエラーでプログラムを終了させる選択が有効である一方で、ミッションクリティカルなシステムでは、単純に処理を継続または停止するのではなく、安全側停止、縮退運転、処理隔離、フェイルオーバーなど、障害時の振る舞いを事前に定義する必要があります。
その違いは、設計書にて明示しない限りAI側では判断が難しい要素となります。

セキュリティに関する品質に関しても例外処理と考え方は同様となります。
