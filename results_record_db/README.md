...
ソフトウェアの品質の話だけでセミナーが成り立つほどの領域ですので、簡単な表で整理します。
今回の利用用途を元に考えた場合、**今回のプログラムが目指すレベルは3に相当**します。
ただし、実運用を意図したものではなく、セミナー向けに構築するという前提のため**レベル2の品質を満たすこと**を要件とします。

AIコーディングを用いてソフトウェアの品質を高める場合、用途の明示化やテスト要件の明示化などをおこなったうえで、アウトプットされたコードに対して、品質の改善を目的としたアクションが必要です。
codexでコード生成する際に複数案を提示させて妥当な実装を選択する進め方もありますが、作成したコード差分をchatGPTやclaude code、code rabbitなど
他のAIサービスにレビューしてもらい、多視点からの評価を元に実装を固めるアプローチも有効です。
