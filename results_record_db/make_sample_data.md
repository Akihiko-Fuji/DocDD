...
> 本文書から生成する氏名、受注番号、製品名、時刻、作業実績はすべて教材用の架空データとし、実在する個人・企業・業務記録を使用しないこと。
...
2. **生成される正常系CSV**
   - `results_record_db/sample_data/INTASM_HanaYamada_202601.csv`
   - `results_record_db/sample_data/EXTASM_MunekiYoshimura_202601.csv`
   - `results_record_db/sample_data/EXTASM_ShuheiYamashita_202601.csv`
   - `results_record_db/sample_data/EXTASM_ToshioAndo_202601.csv`
   - `results_record_db/sample_data/SHIPCHK_202601.csv`
   - `results_record_db/sample_data/order_product_master.csv`

3. **生成される正規化後の期待結果CSV**
   - `results_record_db/sample_expected_work_log.csv`

4. **生成される異常系CSV（reject用）**
   - `results_record_db/sample_data/INTASM_HanaYamadaInvalid_202601.csv`
   - `results_record_db/sample_data/EXTASM_MunekiYoshimuraInvalid_202601.csv`
   - `results_record_db/sample_data/SHIPCHK_202601_invalid.csv`
...
- 3ライン
- 標準サイクル: **240秒/台**
- ばらつき: 中程度（例: -10-+40秒程度）最頻値に対して非対称なばらつき。外れ値を含む
- 少し待ち時間やライン差があってよい


### 外装組立・出荷検査の作業者能力差（デモ可視化用）
- 外装組立（EXTASM）は、次の3名ファイルを使い、**ファイル別レコード件数**で能力差を表現すること。
  - `MunekiYoshimura`: 20.0%（比率 0.6）
  - `ShuheiYamashita`: 43.3%（比率 1.3）
  - `ToshioAndo`: 36.7%（比率 1.1）
- 出荷検査（SHIPCHK）は、`inspector_name` を次の4名のみで構成し、**担当件数比**で能力差を表現すること。
  - `加藤葵`: 17.5%（比率 0.7）
  - `小林陽`: 27.5%（比率 1.1）
  - `田中玲`: 35.0%（比率 1.4）
  - `鈴木ミカ`: 20.0%（比率 0.8）
- いずれも日別集計で不自然な偏りが出ないように、日単位で比率近似しつつ全期間合計を合わせること。
- 既存レコードの `order_no`・時刻・品質系列（`error_code` / `ng_total` 等）は変更しないこと。
...
- どのように `sample_expected_work_log.csv` を作ったか
- どの異常データが reject 想定か
...
3. 正規化後期待結果 `sample_expected_work_log.csv` が生成される
4. 異常系CSVが3種類生成される
...
本書の既存要件を維持したうえで、現行の `sample_data/generate_sample_data.py` と配布CSVは次の条件へ統一する。

- `order_no` は `ORD-YYMMDD-NNN` 形式とする
- `order_no` は前後空白だけを除去し、ハイフン・記号・英字大小を保持する
- 正常3工程、`order_product_master.csv`、`sample_expected_work_log.csv` で同じ `order_no` を使用する
- 旧形式や正常ログから参照されない製番を補助マスタへ残さない
- 19営業日、日別175〜275台の範囲を守る
- 生成スクリプトを正本とし、固定乱数シードから同じCSVを再生成できること

現行生成値は、19営業日、日別194〜244台、総4,321製番である。
外装組立と出荷検査の作業者能力差は、本書に記載した比率を維持する。
