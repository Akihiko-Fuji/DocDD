...
自然言語に含まれる曖昧さを減らし、AIが実装候補を生成・修正できる粒度まで要求や仕様を整え、その実装結果を人間が検証可能な状態に保つための手法です。
...
また、今回の開発のレギュレーションとして、『自らがプログラム本体を直接修正しないこと』を挙げています。
当然ながら自分で修正したほうが早い場面もあるのですが、これは意図的にそうしていません。
非ITエンジニアを対象としているため、ドキュメントを書くことによって修正を実施しています。

また、あくまで『セミナー実演向けに、一般的な事象に対して様々な要素をデフォルメしてデザイン』しているため、業務稼働で必要な要件の一部は考慮していません。
