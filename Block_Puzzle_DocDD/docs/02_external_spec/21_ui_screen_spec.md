...
6. 画面で案内するキーは `22_input_operation_spec.md` と同一にする
...
## 11. T-Spin 表示方針
T-Spin 採用に伴い、成立時にはプレイ画面またはリザルト表示で `T-SPIN` の文字列、または同等に成立結果を識別できる表示を行わなければならない。ただし、派手な演出は必須としない。

---

## 12. Acceptance-Driven 受入観点
1. A-TYPE 主軸の最小画面群が揃っていること
2. 開始設定画面のレベル変更が `Left / Right` で一意に読めること
3. Hold 枠、Hard drop 表示が存在しないこと
4. NEXT 1 個、SCORE、LINES、LEVEL がプレイ画面で確認できること
5. 一時停止とゲームオーバーで通常プレイ入力が無効であること
6. 画面案内キーが `22_input_operation_spec.md` と矛盾しないこと

---

## 13. 変更履歴
- 2026-03-23: 開始設定画面入力を `Left / Right` に統一し、PC キー表記を入力仕様へ整合させた
- 2026-03-23: Acceptance-Driven 観点の受入条件を補強
