...
| ゲームオーバー | A, START, B（PC では ESC 可） | Left, Right, Down, SELECT | A / START で再試行、B でタイトルへ戻る |
...
- 左右移動は押下直後の 1 回を必ず受理し、その後は初回遅延を経て一定間隔でリピートする
- 既定実装値は「初回遅延 8 frames、連続リピート間隔 3 frames」とする
...
### 8.1 既定実装の押下継続パラメータ
- `Left/Right` は押下初回フレームを受理し、**8 フレーム目以降は 3 フレームごと**にリピートする
- `Down` は押下中の各フレームで `InputSnapshot` に保持状態として含める。入力層で3フレームごとのパルスへ間引いてはならない
- ゲーム進行層は独立したソフトドロップカウンタを進め、押下継続3フレームごとに1行の落下を試みる
- `Down` 押下中は通常落下カウンタを停止し、離放後は押下前の残り値から通常落下を再開する。離放だけを理由に通常落下カウンタを初期化してはならない
- これらの値は既定実装値であり、変更時は `36_input_timing_design.md` と同時に更新する
...
## 13. 変更履歴
- 2026-07-17: Down をフレームごとの保持入力として定義し、3フレーム周期の判定責務をゲーム進行層へ移動。ゲームオーバー時の A 再試行を明文化
- 2026-05-17: 実装追従として、BACK(Escape)の扱いと押下継続パラメータ（左右DAS/ARR相当、Down間引き）を明文化
