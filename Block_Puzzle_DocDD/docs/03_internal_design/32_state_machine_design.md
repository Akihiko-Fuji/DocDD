...
7. 入力優先順は `START -> 回転 -> 左右 -> Down -> 自動落下` で固定する
...
## 15. Diagram-Driven / Acceptance-Driven 受入観点
1. タイトル、設定、プレイ、一時停止、ゲームオーバーの遷移が一貫していること
2. T-Spin 判定の責務位置が明確であること
3. B-TYPE が予約状態として整理され、現行 build では遷移しないことが読めること
4. 一時停止中にプレイ進行が止まることが設計上読めること
5. entry / exit の主要責務が読み取れること
6. プレイフレーム順が `36_input_timing_design.md` と一致していること

---

## 16. 変更履歴
- 2026-03-23: 入力優先順と再開直後のバッファ方針を ADR と整合させた
- 2026-03-23: 予約状態に「現行 build では遷移不可」を明記した
