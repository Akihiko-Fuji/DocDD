...
| PL-ARE | 出現待ち | 通常10フレーム、ライン消去時は消去待ち後に10フレーム待機 | AREタイマが0 |
...
- Down中はソフトドロップ専用タイマだけを進め、通常落下タイマは凍結する
- 固定後はライン消去演出待ちとAREを完了してから次ピースを出現させる
...
- 2026-07-16: 実装済みの独立ソフトドロップタイマとPL-AREを状態遷移へ反映
