...
本書は、外部仕様で定義された振る舞いを、**実装しやすく試験しやすい内部契約**へ分解するための設計文書である。ここでのモジュール名・公開契約名は文書上の責務ラベルであり、Python 風コード断片そのものを正本化するものではない。
...
6. 契約は実装言語非依存ではなく**実装言語への依存を下げる**ことを目標とし、class / struct / namespace / package / module などの具体化は採用言語へ委ねる
...
- 本章のシグネチャは擬似コード記法であり、Python / C# / Java / C++ / Rust / Kotlin の各言語では命名規約・可視性・戻り値表現へ読み替えてよい。
- 正本は「どの責務をどの入力と出力で分離するか」であり、メンバー関数・静的関数・トレイト実装・インターフェース実装などの形式は拘束しない。
