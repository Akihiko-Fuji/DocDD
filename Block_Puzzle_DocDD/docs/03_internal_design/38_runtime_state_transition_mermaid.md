...
stateDiagram-v2
    [*] --> ST_BOOT : 起動
    ST_BOOT : ST-BOOT<br/>起動準備
    ST_TITLE : ST-TITLE<br/>タイトル
    ST_SETUP_A : ST-SETUP-A<br/>A-TYPE開始設定
    ST_PLAY : ST-PLAY<br/>プレイ中
    ST_PAUSE : ST-PAUSE<br/>一時停止
    ST_GAMEOVER : ST-GAMEOVER<br/>ゲームオーバー/リザルト
    ST_MODE_SELECT : ST-MODE-SELECT<br/>モード選択(予約)
    ST_SETUP_B : ST-SETUP-B<br/>B-TYPE設定(予約)
...
stateDiagram-v2
    [*] --> PL_SPAWN
    PL_SPAWN : PL-SPAWN<br/>出現
    PL_ACTIVE : PL-ACTIVE<br/>操作/落下
    PL_LOCK_CHECK : PL-LOCK-CHECK<br/>接地/固定判定
    PL_CLEAR : PL-CLEAR<br/>ライン消去
    PL_SCORE : PL-SCORE<br/>得点処理
    PL_NEXT : PL-NEXT<br/>次ピース準備
    PL_END_CHECK : PL-END-CHECK<br/>終了判定

    PL_SPAWN --> PL_ACTIVE : 出現成功
    PL_SPAWN --> ST_GAMEOVER : 出現不能
    PL_ACTIVE --> PL_ACTIVE : 左右移動 / 回転 / ソフトドロップ / 自動落下
