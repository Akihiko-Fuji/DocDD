...
- pygame正本実装の盤面セルは24×24pxとし、`playfield_cells` は10列×18行=240×432pxとする。
...
## 3.3 pygame 実装レイアウト定数（2026-07-16）

`src/DocDD_coding/falling_block_puzzle/renderer.py` は、GB風背景アートと実際の画像寸法に合わせて以下の固定座標系を使用する。
...
補足:
- 本節は pygame 実装の正本値を示す。将来アセット差し替え時は本書と実装を同時更新する。
- 10×18 盤面サイズ自体は変更しない。
...
### 6.1 盤面
- 可視10×18を固定表示し、pygame実装のセルサイズは24pxとする
- 左上原点は画面座標 `(96, 72)` とする
...
### 基準サイズ
- ブロック素材の原寸は32×32px、pygame実装の盤面セルは24×24pxとする
