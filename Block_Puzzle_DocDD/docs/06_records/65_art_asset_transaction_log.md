...
| ファイル | 想定用途 | 状態 |
|---|---|---|
| `puzzle.png` / `puzzle-menu.png` | SETUP-A / タイトル背景 | 利用可・採用先確定 |
...
|---|---|---|---|---|
| P1 | `-` | `A-TYPE`, `B-TYPE`, `T-SPIN` | 利用可（`hyphen.png`） | 完了 |
| P1 | ` ` (space) | `GAME OVER`, `NEXT OFF` | 利用可（`space.png`） | 完了 |
...
| 2026-07-16 | 実装整合 | 現物寸法を確認し、fontset 16×16、sidebar 252×648、sidewall 32×24を正本化 | 描画時の歪みと文書不整合を解消 |
| 2026-07-16 | P1完了 | `hyphen.png` と `space.png` の存在・読込を確認 | 必須文言をfontsetのみで表示可能 |
| 2026-07-22 | 配置修正 | `brick5center.png` / `brick5end.png` をIピースの長軸と端部方向に合わせて回転描画 | 横・縦、操作中・固定後・NEXTで端部の閉じた辺を外側へ統一 |

---

## 5. 次アクション

1. P2候補の `:`, `/`, `(`, `)` は、実際に画面文言へ採用するときに追加制作する
2. artwork差し替え時は現物寸法と透過有無を本書へ追記する
