...
## 2026-07-22
- Iピースの `brick5center.png` / `brick5end.png` を長軸方向へ回転し、左右端・上下端の閉じた辺が外側を向くよう修正した。
- 固定済みIピースへ6種類の方向別セル役割を保存し、ライン消去後も画像の向きを維持するようにした。

## 2026-07-16
- SCR-003 の画像レイアウトを現物寸法へ整合させた。`sidebar.png` の縦横比を維持し、`sidewall.png` を32×24px比で配置した。
- HUD数値を右揃えに変更し、SCORE / LEVEL / LINES の桁数変化によるラベル重なりを防止した。
- SCR-002 と SCR-005 で、仕様どおり A 入力でも開始・再試行できるようにした。
- タイトル表示時の不要なプレイセッション生成を廃止し、開始前の乱数消費を防止した。
- ソフトドロップと通常落下を独立タイマ化し、Down時の二重落下と接地後停止を防止した。
- 固定後のARE 10フレームと、ライン消去20フレーム後のARE 10フレームを実装した。
- 盤面24px・fontset16pxという現物実装値を画像仕様・UI仕様・描画設計へ反映した。
- 移動・回転後のIピース端画像を絶対座標ではなく現在形状の両端から判定するよう修正した。
- PAUSE / GAME OVERの案内を盤面上端より上へ収め、プレイフィールドとの重なりを解消した。
