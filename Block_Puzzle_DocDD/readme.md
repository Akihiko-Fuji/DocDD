...
7. [主な参照先](#7-主な参照先)
8. [バイブコーディングと DocDD 実装の比較](#8-バイブコーディングと-docdd-実装の比較)
...
## 8. バイブコーディングと DocDD 実装の比較

`src/vibe_coding/vibe_code_tetris.py` は、次の短い依頼だけをもとに生成した比較用実装です。

> Python標準ライブラリを使って、軽量なテトリス風落下パズルを1ファイルで作成してください。

一方、`src/DocDD_coding/` は、`docs/` の要求・外部仕様・内部設計・試験仕様を一次情報として制作した正本実装です。両者の違いは、単純な機能数だけではなく、**未指定事項をAIの判断へ委ねるか、正しい振る舞いを文書で固定するか**にあります。

以下は、2026年7月18日時点の `main` ブランチを基準にした比較です。

| 比較項目 | `vibe_code_tetris.py` | `DocDD_coding` |
|---|---|---|
| 開発指示 | 「Python標準ライブラリを使って、軽量なテトリス風落下パズルを1ファイルで作成してください。」の1文 | 要求、外部仕様、内部設計、試験仕様など複数の文書 |
| 指定の具体性 | 使用言語・軽量性・1ファイル以外はほぼ指定していない | 盤面、操作、回転、得点、画面、状態遷移、タイミングなどを明文化 |
| 仕様の確定度 | **揺らぐ仕様**。一般的なテトリス像とAIの判断から仕様が決まる | **文書によって固定された仕様**。定義済み範囲はAIの判断で変更しない |
| 同じ指示での再生成 | 操作方法、色、画面サイズ、得点、ゲームオーバー処理などが変わる可能性がある | 同じ正本文書を参照する限り、同じ振る舞いを再現しやすい |
| 未指定事項の扱い | AIがその場で補完し、生成されたコードが事実上の仕様になる | 未定義事項、補完判断、非採用機能、既知の制限を文書に残す |
| 仕様変更の扱い | コードだけを変更できるため、元の意図との差が分かりにくい | 文書・実装・テストを同時に更新し、変更理由と影響範囲を追跡する |
| Pythonファイル数 | **1ファイル** | **実装16ファイル＋テスト11ファイル＝27ファイル** |
| 実装の総行数 | **225行** | **930行** |
| 実質的なコード行数 | **94行** | **711行** |
| テストコード | なし | **11ファイル、440行**（実質303行） |
| 実装＋テスト合計 | **1ファイル、225行** | **27ファイル、1,370行** |
| 使用ライブラリ | Python標準の `tkinter` のみ | `pygame-ce` を使用 |
| 構造 | 1クラスに入力、状態、ルール、描画を集約 | 盤面、ピース、入力、状態遷移、得点、描画などを分割 |
| 盤面 | 10列×18行 | 10列×18行 |
| ピース生成 | 7種類からランダム選択 | 7種類から選択。seed指定による再現も可能 |
| 座標・形状定義 | 独自の簡易定義で、仕様上の座標系との違いがある | 座標軸、出現位置、回転状態、占有マスを文書で定義 |
| 回転 | 時計回りのみ | 時計回り・反時計回り |
| 自動落下 | 常に400ミリ秒間隔 | レベルに応じて落下速度が変化 |
| ライン消去 | 完成行を即座に削除 | 点滅表示、消去待ち、ARE待機を経て次ピースを出現 |
| 得点・レベル | なし | ライン、レベル、ソフトドロップ、T-Spinから得点を計算 |
| T-Spin | なし | 成立条件と得点を明文化して実装 |
| NEXT表示 | なし | 次ピースを表示し、表示・非表示を切替可能 |
| 画面状態 | プレイ画面のみ | TITLE、SETUP_A、PLAY、PAUSE、GAMEOVER |
| ゲームオーバー | 説明なしで自動リセット | ゲームオーバー画面から再挑戦またはタイトルへ戻る |
| 表示 | 黒背景、白・シアンの矩形 | 640×576のゲーム機風画面、画像素材、ビットマップフォント |
| 素材異常への対応 | 外部素材なし | 読み込み失敗時に矩形・標準フォントへフォールバック |
| 非採用機能 | 実装されなかった機能の理由は残らない | ハードドロップ、ホールド、wall kickなどを非採用と明示 |
| 自動検証 | なし | 盤面、回転、得点、状態遷移、描画などを `pytest` で検証 |
| 適した用途 | アイデア確認、短時間の試作、バイブコーディングの実演 | 仕様再現、品質保証、継続改修、複数人での開発 |

> 行数の数え方:
> - 「総行数」には、空行、コメント、docstringを含む。
> - 「実質的なコード行数」は、空行、コメント、docstringを除いた値とする。
> - 文書、画像素材、設定ファイルは集計対象に含めない。

`vibe_code_tetris.py` の出来が悪いという意味ではありません。短い依頼に対して、落下パズルとして遊べるものを極めて小さく作るという目的には十分に応えています。

一方、DocDD実装が追加しているのは、単なる機能量ではありません。バイブコーディング版ではAIへ委ねられていた操作、回転、得点、画面遷移、待機時間、非採用機能などの判断を、仕様・モジュール責務・テストへ変換しています。

> バイブコーディングは「それらしい動くもの」を短時間で作る。
>
> DocDDは「何を正しいとするか」を文書で固定し、実装とテストで再現する。

したがって、コード量の増加は単なる機能追加ではなく、**曖昧だった判断を仕様・責務・検証へ変換した量**として捉えることができます。
